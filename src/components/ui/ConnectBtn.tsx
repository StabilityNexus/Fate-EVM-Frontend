...
import { logWalletEvent, logWalletError, WalletEvent } from '@/lib/walletLogger';
...
function ModalOverlay({ onClose, children, labelledById, isOpen }: { onClose: () => void, children: React.ReactNode, labelledById: string, isOpen: boolean }) {
...
export default function ConnectBtn() {
  ...
  const { connectors, connect, isPending: isConnectPending, error: connectError, reset: resetConnect, variables: connectVariables } = useConnect();
  const { disconnect } = useDisconnect();
  const chainId = useChainId();
  const { chains, switchChainAsync, isPending: isSwitchPending } = useSwitchChain();
  ...
  useEffect(() => {
    const auditConnectors = async () => {
      ...
      for (const connector of connectors) {
        ...
        if (connector.id === 'walletConnect' || connector.id === 'coinbaseWalletSDK') continue;

        try {
          const provider = (await connector.getProvider()) as Record<string, unknown> | undefined | null;
          // Rule 2: Skip null/undefined providers (phantom wallets)
          if (!provider) continue;

          // Detect MetaMask identity natively or via ID
          if (connector.id.toLowerCase().includes('metamask') || provider.isMetaMask) {
            hasMetaMask = true;
          }
      ...
      const uniqueValid = Array.from(new Map(valid.map(c => [c.id, c])).values());
  ...
  const networkState = !isConnected && !isConnecting ? 'DISCONNECTED'
  ...
  useEffect(() => {
    if (isConnected && isConnectModalOpen) {
      logWalletEvent(WalletEvent.CONNECT_SUCCESS, { address });
  ...
  const handleConnect = useCallback((connector: Connector) => {
    logWalletEvent(WalletEvent.CONNECT_INITIATED, { connectorId: connector.id });
    resetConnect();
    connect({ connector }, {
      onError: (error) => {
        logWalletError(WalletEvent.CONNECT_FAILED, error);
      }
    });
  ...
  const handleDisconnect = useCallback(() => {
    ...
    disconnect(undefined, {
      onSuccess: () => logWalletEvent(WalletEvent.DISCONNECT_SUCCESS, { address })
    });
    setIsAccountModalOpen(false);
  }, [disconnect, address]);
  ...
  const handleSwitchChain = async (cId: number) => {
    if (!isSupportedChain(cId)) {
      logWalletError(WalletEvent.CHAIN_SWITCH_FAILED, new Error(`Unallowed attempt to switch to unsupported chain: ${cId}`));
      return;
    }

    logWalletEvent(WalletEvent.CHAIN_SWITCH_INITIATED, { targetChainId: cId, currentChainId: chainId });
    ...
    try {
      ...
      if (error?.code === 4001 || msg.toLowerCase().includes('user rejected')) {
        logWalletEvent(WalletEvent.CHAIN_SWITCH_CANCELLED, { targetChainId: cId });
      } else {
  ...
  } else if (networkState === 'CONNECTED_UNSUPPORTED' || networkState === 'CONNECTED_UNKNOWN') {
    walletUI = (
      <motion.button
        whileHover={{ y: -1 }}
        whileTap={buttonTap}
      ...
      >
        <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-full bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-400 hover:opacity-80 transition-opacity cursor-pointer">
          ...
          <span className="text-xs font-semibold whitespace-nowrap">
            {networkState === 'CONNECTED_UNKNOWN' ? `Unknown (${chainId})` : 'Wrong Network'}
          </span>
        </div>
        <span className="text-sm font-bold text-zinc-900 dark:text-white px-1 font-mono tracking-tight">{truncateAddress(address)}</span>
    ...
    walletUI = (
      ...
      >
        <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-full bg-zinc-100 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 hover:opacity-80 transition-opacity cursor-pointer">
          ...
          <span className="text-xs font-semibold whitespace-nowrap">{activeChain?.name}</span>
        </div>
        <span className="text-sm font-bold text-zinc-900 dark:text-white px-1 font-mono tracking-tight">{truncateAddress(address)}</span>
        <ChevronDownIcon className="w-4 h-4 text-zinc-400 group-hover:text-zinc-900 dark:group-hover:text-white transition-colors" />
      </motion.button>
  ...
  return (
    <>
      ...
      <ModalOverlay isOpen={isConnectModalOpen} onClose={() => { setIsConnectModalOpen(false); resetConnect(); }} labelledById="connect-modal-title">
        <motion.div variants={modalVariants} className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 w-full max-w-[360px] rounded-3xl overflow-hidden shadow-2xl">
          <div className="p-6">
            <div className="flex justify-between items-center mb-6">
              <h2 id="connect-modal-title" className="text-lg font-bold text-zinc-900 dark:text-zinc-100 tracking-tight">
                Connect Wallet
              </h2>
              <motion.button whileTap={buttonTap} onClick={() => { setIsConnectModalOpen(false); resetConnect(); }} className="p-1.5 text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-100 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-full transition-colors">
                <CloseIcon />
              </motion.button>
            </div>
            {connectError && (
              <div role="alert" className="mb-4 px-4 py-3 rounded-2xl bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/20 text-sm text-red-600 dark:text-red-400">
                {connectError.message.toLowerCase().includes('user rejected') ? 'Connection rejected.' : 'Failed to connect.'}
              </div>
            )}
            <div className="space-y-2">
              {validConnectors.length > 0 ? (
                validConnectors.map((connector) => {
                  ...
                  const isThisPending = isConnectPending && (connectVariables?.connector as Connector)?.id === connector.id;
                  const hasImgError = imgErrors[connector.id] ?? false;
                  return (
                    <motion.button
                      key={connector.id} whileTap={buttonTap} disabled={isConnectPending} onClick={() => handleConnect(connector)}
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div className="relative w-10 h-10 rounded-xl bg-white dark:bg-zinc-950 border border-zinc-100 dark:border-zinc-800 flex items-center justify-center shrink-0 shadow-sm overflow-hidden">
                          {hasImgError ? <span className="text-zinc-400 font-bold select-none text-sm">{info.name.charAt(0)}</span> : (
                            <Image src={info.logo} alt={info.name} fill className="object-contain p-2" onError={() => handleImgError(connector.id)} />
                          )}
                        </div>
                      ...
                      {isThisPending && <div className="w-4 h-4 mr-2 border-2 border-zinc-900 dark:border-zinc-100 border-t-transparent animate-spin rounded-full shrink-0" />}
                    </motion.button>
                  );
                })
              ) : (
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className="relative w-10 h-10 rounded-xl bg-white dark:bg-zinc-950 border border-zinc-100 dark:border-zinc-800 flex items-center justify-center shrink-0 shadow-sm overflow-hidden">
                       <span className="text-zinc-400 font-bold select-none text-sm">W</span>
                    </div>
                    <span className="font-semibold text-[15px] text-zinc-900 dark:text-zinc-100">
                      Install Wallet
                    </span>
      ...
      <ModalOverlay isOpen={isAccountModalOpen} onClose={() => setIsAccountModalOpen(false)} labelledById="account-modal-title">
        <motion.div variants={modalVariants} className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 w-full max-w-[360px] rounded-[28px] overflow-hidden shadow-2xl relative">
          <AnimatePresence mode="wait" initial={false}>
            {viewState === "account" && (
              ...
              >
                <div className="flex justify-between items-center mb-6">
                  <h2 id="account-modal-title" className="text-lg font-bold text-zinc-900 dark:text-zinc-100 tracking-tight">
                    Account
                  </h2>
                  <motion.button whileTap={buttonTap} onClick={() => setIsAccountModalOpen(false)} className="p-1.5 text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-100 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-full transition-colors">
                    <CloseIcon />
                  </motion.button>
                </div>
                <div className="flex flex-col items-center gap-3 mb-8">
                  <motion.div layoutId="wallet-chip-modal-wrapper" className="w-[60px] h-[60px] bg-zinc-50 dark:bg-zinc-900 rounded-2xl border border-zinc-100 dark:border-zinc-800 flex items-center justify-center overflow-hidden shadow-sm">
                    {activeConnector ? (() => {
                      const info = getWalletInfo(activeConnector);
                      const hasErr = imgErrors[activeConnector.id] ?? false;
                      if (hasErr) return <span className="text-zinc-400 font-bold">{info.name.charAt(0)}</span>;
                      return <Image src={info.logo} alt={info.name} width={36} height={36} className="object-contain" onError={() => handleImgError(activeConnector.id)} />;
                    })() : <span className="text-zinc-400">W</span>}
                  </motion.div>
                  <div className="text-center">
                    <p className="text-[22px] font-bold text-zinc-900 dark:text-white font-mono tracking-tight" title={address}>
                    ...
                    >
                      <div className="flex items-center gap-2">
                        <span className={`w-2 h-2 rounded-full ${networkState === 'CONNECTED_UNSUPPORTED' || networkState === 'CONNECTED_UNKNOWN' ? 'bg-red-500 animate-pulse' : 'bg-emerald-500'}`} />
                        <span className="text-sm font-semibold text-zinc-800 dark:text-zinc-200">
                          {networkState === 'CONNECTED_UNSUPPORTED' ? 'Wrong Network' : networkState === 'CONNECTED_UNKNOWN' ? `Unknown (${chainId})` : activeChain?.name ?? 'Loading...'}
                        </span>
                      </div>
                ...
                <div className="flex flex-col gap-2">
                  <div className="grid grid-cols-2 gap-2">
                    <motion.button
                      whileTap={buttonTap} onClick={handleCopyAddress}
                    ...
                    >
                      <CopyIcon copied={copySuccess} /> {copySuccess ? 'Copied!' : 'Copy'}
                    </motion.button>
                    {explorerAddressUrl ? (
                      <motion.a
                        whileTap={buttonTap} href={explorerAddressUrl} target="_blank" rel="noopener noreferrer"
                        className="flex items-center justify-center gap-2 h-[44px] rounded-xl bg-zinc-50 dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors text-sm font-semibold text-zinc-700 dark:text-zinc-300"
                      >
                  ...
                  <motion.button
                    whileTap={buttonTap} onClick={handleDisconnect}
                  ...
                  >
                    <DisconnectIcon className="w-4 h-4 transition-transform group-hover:-translate-x-0.5" /> Disconnect
                  </motion.button>
                </div>
              </motion.div>
            )}
            {viewState === "network" && (
              ...
              >
                <div className="flex items-center gap-3 mb-6">
                  <motion.button whileTap={buttonTap} onClick={() => setViewState("account")} className="p-1.5 -ml-1.5 text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-100 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-full transition-colors">
                  ...
                  <h2 className="text-lg font-bold text-zinc-900 dark:text-zinc-100 tracking-tight">Switch Networks</h2>
                </div>
